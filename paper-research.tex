...
\textit{Lakshmibai National College of Technology (LNCT)}\\
...
(delta-R), requires no iterative optimization and no gradient computation.
...
sparse feature augmentation, classification, educational data mining
\end{IEEEkeywords}
...
Input features are processed in two branches simultaneously.
...
Naive Bayes implicitly assumes $R^{(c)}_{ij} = 0$ for all classes and all
pairs.
This is correct only when features are genuinely conditionally independent.
...
\begin{figure}[!t]
\centerline{\includegraphics[width=\columnwidth]{fig_heatmap.pdf}}
...
The complete code including the evaluation harness and figure generation
scripts is available in the supplementary material.
...
class CSLN(BaseEstimator, ClassifierMixin):
    """
...
The \texttt{fit} method follows Algorithm~\ref{alg:train} exactly.
\texttt{selected\_pairs\_} stores $\Omega_k$ and is used at inference time
inside \texttt{\_enrich}.
Feature standardisation is handled externally via \texttt{StandardScaler}
inside the cross-validation folds to prevent data leakage.
...
\begin{table}[!t]
\caption{Iris ($N$=150, $d$=4, $C$=3). CS-LLN uses $k$=5.
LDA achieves the highest accuracy on this low-dimensional dataset.}
\label{tab:iris}
\begin{center}
\begin{tabular}{lrrr}
\toprule
...
\begin{figure}[!t]
\centerline{\includegraphics[width=\columnwidth]{fig_comparison.pdf}}
...
With $d = 4$, there are only six possible interaction pairs; LDA's
pooled Gaussian assumption is well matched to this clean, low-dimensional
dataset where covariance structure is estimable from 150 samples.
...
\begin{figure}[!t]
\centerline{\includegraphics[width=\columnwidth]{fig_ablation.pdf}}
...
Performance degrades only when $k$ approaches the full pairwise expansion
($k = 78$ out of 435).
...
Performance degrades at $k = 78$ (96.49\%), consistent with the known
result that indiscriminate polynomial expansion introduces multicollinearity.
...
\begin{figure}[!t]
\centerline{\includegraphics[width=\columnwidth]{fig_top_pairs.pdf}}
...
% ─────────────────────────────────────────────────────────────────────────────
\section{Student Academic Performance Study}
\label{sec:student}
...
The practical minimum for stable CPMI and $\Delta R_{ij}$ estimation is
...
Naive Bayes implicitly treats $R^{(c)}_{ij} = 0$ for every class and every
pair.
...
\textit{J.\ Roy.\ Stat.\ Soc.\ Ser.~B}, vol.~74, no.~4,
pp.~745--771, 2012.
